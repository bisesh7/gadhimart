...
// @route   GET /api/reportCarListings/
// @desc    Get all the car listing reports
// @access  ADMIN
router.get("/", (req, res) => {
  ...
  const getData = (documents) => {
    ...
    const pushToData = (document) => {
      data.push({
        carId: document.listingId,
        reportedBy: document.reportedByEmail,
  ...
  ReportListing.find(filterDetail)
...
// @route   GET /api/reportCarListings/:id
// @desc    Get a carlisting report wih given id
// @access  ADMIN
...
// @route   DELETE /api/reportCarListings?query
// @desc    Delete car listing report in query
// @access  ADMIN
router.delete("/", (req, res) => {
  ...
  if (typeof req.query.filter !== "undefined") {
    ...
    ReportListing.find({ vehicleType: "Car" })
      .where("_id")
      .in(id)
      .exec((err, documents) => {
        if (err) {

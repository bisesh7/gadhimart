...
// @route   GET /api/savedMotorcycles/
// @desc    Get all the saved motorcycles
// @access  ADMIN
router.get("/", (req, res) => {
  ...
  const getData = (documents) => {
    ...
    const pushToData = (document) => {
      data.push({
        motorcycleId: document.vehicleId,
        id: document.id,
...
// @route   GET /api/savedMotorcycles/:id
...
// @route   DELETE /api/savedMotorcycles?query
// @desc    Delete saved motorcycles in query
// @access  ADMIN
router.delete("/", (req, res) => {
  ...
  if (typeof req.query.filter !== "undefined") {
    ...
    SavedVehicles.find({ vehicleType: "Motorcycle" })
      .where("_id")
      .in(id)
      .exec((err, documents) => {
        if (err) {
        ...
        // Deleting the saved motorcycles
        Promise.all(promises)
          ...
          .catch((err) => {
            return res.status(500).json({
              success: false,
              err: "serverError",
              msg: "Error deleting the saved motorcycles.",
            });
          });
      });
  }
